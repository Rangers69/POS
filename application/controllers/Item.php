...
class item extends CI_Controller
{
            ...
            if ($this->item_m->check_barcode($post['barcode'], $post['id'])->num_rows() > 0) {
                ...
                if (@$_FILES['image']['name'] != null) {
                    if ($this->upload->do_upload('image')) {
                        ...
                        if ($this->db->affected_rows() > 0) {
                            $this->session->set_flashdata('success', 'Data berhasil ubah');
                        }
                        redirect('item');
                    } else {
                        $error = $this->upload->display_errors();
    ...
    function zubair()
    {
        $qrCode = new Endroid\QrCode\QrCode('I Love You Istriku, selamat ya sudah 3 bulan menjadi seorang Ibu semoga kita selalu diberikan kesabaran dan keberkahan dalam mengasuh dan mendidik anak kita aamiin 
        
Tertanda,
Laki-laki yang paling mencintaimu :*');
